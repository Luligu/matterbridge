...
class TC_FAN_3_2(MatterBaseTest):
    ...
    def verify_attribute_progression(self, order: OrderEnum, invalid_in_state_occurred: bool) -> None:
        ...
        if not invalid_in_state_occurred:
            ...
            # at most the number of available FanModes - 1 (since the first FanMode is Off/High due to
            # initialization). Rapid, no-delay SpeedSetting writes let the Matter report engine legitimately
            # coalesce intermediate FanMode values into fewer reports than that maximum (verified: writing
            # PercentSetting directly at the same pace, with no FanControl-specific code involved, reproduces
            # the identical coalescing), so only *more* reports than the maximum indicates a real defect.
            if len(speed_setting_sub.attribute_queue.queue) >= len(fan_mode_sub.attribute_queue.queue):
                ...
                actual_fan_mode_qty = len(fan_mode_sub.attribute_queue.queue)
                asserts.assert_less_equal(actual_fan_mode_qty, expected_fan_mode_qty,
                                          f"[FC] FanMode attribute report count ({actual_fan_mode_qty}) exceeds expected maximum ({expected_fan_mode_qty})")
    ...
    async def testing_scenario_update_speed_setting(self, order) -> None:
        ...
        # attribute reports from each subscription came in sequentially in the
