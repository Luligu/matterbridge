...
class TC_DRLK_2_9(MatterBaseTest, DRLK_COMMON):
    ...
    async def set_credential_cmd(self, credential_enum: drlkcluster.Enums.CredentialTypeEnum, statuscode, credentialIndex,
        ...
        try:
            ...
            log.exception(e)
            actual_status = e.clusterStatus if e.hasClusterStatus else e.status
            if statuscode != custom_status_code:
                asserts.assert_equal(actual_status, statuscode, f"Unexpected error returned: {e}")
            else:
                asserts.assert_in(actual_status, (2, 3), f"Unexpected error returned: {e}")
            return -1
