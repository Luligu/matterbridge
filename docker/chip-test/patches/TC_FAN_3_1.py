...
class TC_FAN_3_1(MatterBaseTest):
    ...
    def steps_TC_FAN_3_1(self):
        return [TestStep(1, "[FC] Commissioning already done.", is_commissioning=True),
                ...
                TestStep(4, "[FC] TH tests the following scenario: - Attribute to update: PercentSetting - Attribute to verify: PercentSetting, FanMode and SpeedSetting (if present) - Update order: Ascending. Actions: * Initialize the DUT to `FanMode` Off and read back the value to verify written value. * Individually subscribe to the `PercentSetting`, `FanMode`, and `SpeedSetting` (if supported) attributes * Update the value of the `PercentSetting` attribute iteratively, in ascending order, from 1 to 100.",
                         "For each PercentSetting update, the DUT shall return either SUCCESS or INVALID_IN_STATE. After all updates have been performed, verify that the value of the attribute reports from the subscription of each attribute came in sequentially in ascending order (each new value greater than the previous one)."),
    ...
    async def wait_for_triggered_reports(self, attr_to_update, value_written) -> None:
        """Blocks until the reports triggered by a successful attribute write are delivered.

        Waits for attr_to_update's own report of value_written, then reads back the other
        subscribed FanControl attributes and waits for their reports of the read values when
        the write cascaded into them. Serializing on delivered reports this way, instead of
        firing the whole value_range as fast as possible, keeps matter.js's report engine from
        coalescing intermediate values together — which otherwise makes the post-loop report
        count/progression checks race the DUT's own report delivery.
        """
        subs = {sub._expected_attribute: sub for sub in self.subscriptions}
        self.wait_for_latest_report_value(subs[attr_to_update], value_written)
        self.last_values[attr_to_update] = value_written

        for attr, sub in subs.items():
            if attr == attr_to_update:
                continue
            value_read = await self.read_setting(attr)
            if value_read != self.last_values[attr]:
                self.wait_for_latest_report_value(sub, value_read)
                self.last_values[attr] = value_read

    def log_results(self) -> None:
        for sub in self.subscriptions:
            log.info("[FC] - %s Sub -", sub._expected_attribute.__name__)
            for q in list(sub.attribute_queue.queue):
    ...
    def verify_attribute_progression(self, order) -> None:
        ...
        for sub in self.subscriptions:
            values = [q.value for q in list(sub.attribute_queue.queue)]
    ...
    async def testing_scenario(self, attr_to_update, order) -> None:
        ...
        # Baseline for detecting which subscribed attributes a write actually cascades into
        self.last_values = {sub._expected_attribute: await self.read_setting(sub._expected_attribute) for sub in self.subscriptions}
        ...
        for value_to_write in value_range:
            write_status = await self.write_and_verify_attribute(attr_to_update, value_to_write, allowed_update)
            if write_status == Status.Success:
                await self.wait_for_triggered_reports(attr_to_update, value_to_write)
