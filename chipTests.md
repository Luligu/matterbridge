...
- **ClosureControl: found and fixed two real bugs in `MatterbridgeClosureControlServer` — no simulated
  movement completion, and a constraint-check ordering bug.** Fixed in
  `packages/core/src/devices/closure.ts` (v1.1.0).
  - **No movement/calibration completion.** `moveTo()`/`calibrate()` set `MainState` to `Moving`/`Calibrating`
    synchronously on command receipt but nothing ever transitioned it back to `Stopped`, updated
    `OverallCurrentState` to match `OverallTargetState`, decremented `CountdownTime`, or emitted
    `MovementCompleted`/`SecureStateChanged` — every test that sends `MoveTo`/`Stop` and then waits (via
    subscription) for the resulting state failed on timeout (45s for most tests; `TC_CLCTRL_4_4.py` falls
    back to a 259200s/3-day wait when no `--timeout` is passed, since it reads `CountdownTime` as its own
    timeout bound — the container had to be manually recovered from this the first time). Fixed by having
    `moveTo()`/`calibrate()` schedule a 5-second timer (`movementSimulationMs`) that simulates the operation
    completing: updates the relevant attributes and emits the events via the existing `Closure.setState()`/
  ...
  - **Constraint checks ran after state checks.** `TC_CLCTRL_4_3.py` sends `MoveTo(position=6)` (not a valid
    `TargetPositionEnum` member) while latched and expects `CONSTRAINT_ERROR`, but got `INVALID_IN_STATE`
    instead: the latch-based state check ran before any range validation of `request.position`, so a
    malformed field was masked by an unrelated business-state error. Per the general Interaction Model rule
    that a command field constraint violation must be rejected ahead of any other command- or
    cluster-specific validation, `moveTo()` now validates `Position` (enum range), `Latch` (boolean type),
    and `Speed` (enum range) as the very first checks in the command handler, before the choice-conformance
    and state checks.

  Also added a fourth `Closure` demo endpoint, `Closure Complete` (8054, see the endpoint map above), since
  `TC_CLCTRL_3_1`/`4_1`/`5_1` each gate real coverage of one or more sections behind a feature (Calibration,
  Ventilation, Pedestrian) that the plain `Closure` endpoint (805) doesn't support — those sections always
  skipped via the test's own live `FeatureMap` read rather than actually exercising the underlying code, so
  those three `chipTests.json` entries now point at 8054 instead of 805, using a new
  `docker/chip-test/closure-control-complete.pics` (adds `F04`/`F05`/`F06`/`C02.Rsp` for
  Ventilation/Pedestrian/Calibration/Calibrate on top of `closure-control.pics`'s base set) rather than
  editing `closure-control.pics` itself, which stays as the accurate default-feature-set (805) profile.
  `TC_CLCTRL_2_1`/`4_2`/`4_3`/`4_4`/`6_1` don't branch on those three features and stay on 805 with the
  original PICS file.
