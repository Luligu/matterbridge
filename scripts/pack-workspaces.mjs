...
function usage() {
  return [
    'Usage: node scripts/pack-workspaces.mjs --tag <dev|edge|git|local|latest> [--scope <prefix>] [--dry-run]',
    '',
    'What it does:',
    ...
    '  3) Runs scripts/bundle-workspaces.mjs (sync workspace versions + scoped deps)',
    '  4) Runs npm run buildProduction',
    '  5) Temporarily removes devDependencies, scripts from all package.json files',
    '  6) Pack the monorepo (or --dry-run pack)',
    '  7) Restores original package.json files from the in-memory backup',
    '',
    'Options:',
    '  --tag, -t    Required. One of: dev | edge | git | local | latest',
    '  --scope, -s  Dependency name prefix to sync/bundle (default: @matterbridge)',
    '  --dry-run, -n Only applies to pack (adds npm pack --dry-run)',
  ].join('\n');
}
...
if (tag !== 'dev' && tag !== 'edge' && tag !== 'git' && tag !== 'local' && tag !== 'latest') {
  console.error(usage());
  process.exitCode = 1;
...
try {
  ...
  runCmd(repoRoot, false, 'node', ['scripts/bundle-workspaces.mjs', '--scope', scopePrefix]);
