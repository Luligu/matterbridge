...
function QRDiv({ id }: QRDivProps) {
  ...
  if (debug || localDebug) console.log(`QRDiv loading with id = "${id}" and matter:`, matter);

  // Request server data when id changes
  useEffect(() => {
    if (debug || localDebug) console.log(`QRDiv id effect "${id}"`);
    ...
    if (id) {
      if (debug || localDebug) console.log(`QRDiv id effect sending data request for storeId "${id}"`);
      sendMessage({ id: uniqueId.current, sender: 'QRDiv', method: '/api/matter', src: 'Frontend', dst: 'Matterbridge', params: { id: id, server: true } });
    } else {
      if (debug || localDebug) console.log('QRDiv id effect setting matter to null');
  ...
  useEffect(() => {
    const handleWebSocketMessage = (msg: WsMessageApiResponse) => {
      if (debug || localDebug) console.log('QRDiv received WebSocket Message:', msg);
      if (msg.method === 'refresh_required' && msg.response.changed === 'matter' && msg.response.matter) {
        if (debug || localDebug) console.log(`QRDiv received refresh_required: changed=${msg.response.changed} for storeId "${msg.response.matter.id}":`, msg.response.matter);
        if (storeIdRef.current === msg.response.matter.id) {
          if (debug || localDebug) console.log(`QRDiv received refresh_required/matter: setting matter data for storeId "${msg.response.matter.id}":`, msg.response.matter);
          if (advertiseTimeoutRef.current) clearTimeout(advertiseTimeoutRef.current);
          ...
          /*
          Removed in matter.js
          if (msg.response.matter.advertising) {
            if (debug || localDebug) console.log(`QRDiv setting matter advertise timeout for storeId "${msg.response.matter.id}":`, msg.response.matter.advertiseTime + 15 * 60 * 1000 - Date.now());
            advertiseTimeoutRef.current = setTimeout(
              () => {
                // Clear advertising state after 15 minutes
                if (advertiseTimeoutRef.current) clearTimeout(advertiseTimeoutRef.current);
                if (debug || localDebug) console.log(`QRDiv clearing advertising state for storeId "${storeIdRef.current}" after 15 minutes`);
    ...
    if (debug || localDebug) console.log('QRDiv webSocket effect mounted');

    return () => {
      removeListener(handleWebSocketMessage);
      if (advertiseTimeoutRef.current) clearTimeout(advertiseTimeoutRef.current);
      advertiseTimeoutRef.current = null;
      if (debug || localDebug) console.log('QRDiv webSocket effect unmounted');
    };
  }, [addListener, removeListener]);

  const handleStartCommissioningClick = () => {
    if (debug || localDebug) console.log(`QRDiv sent matter startCommission for node "${matter?.id}"`);
    // v8 ignore next line -- just defensive check
    if (!matter) return;
  ...
  const handleStopCommissioningClick = () => {
    if (debug || localDebug) console.log(`QRDiv sent matter stopCommission for node "${matter?.id}"`);
    // v8 ignore next line -- just defensive check
    if (!matter) return;
  ...
  const handleAdvertiseClick = () => {
    if (debug || localDebug) console.log(`QRDiv sent matter advertise for node "${matter?.id}"`);
    // v8 ignore next line -- just defensive check
    if (!matter) return;
  ...
  const handleRemoveFabric = (fabricIndex: number) => {
    if (debug || localDebug) console.log(`QRDiv sent matter removeFabric for node "${matter?.id}" and fabricIndex ${fabricIndex}`);
    // v8 ignore next line -- just defensive check
    if (!matter) return;
  ...
  const handleCopyManualCode = async () => {
    ...
    try {
      ...
      if (debug || localDebug) console.log('Manual pairing code copied to clipboard');
    } catch (e) {
  ...
  if (!matter || !online) {
    if (debug || localDebug) console.log('QRDiv rendering undefined state');
    return null;
  } else if (!matter.online) {
    if (debug || localDebug) console.log('QRDiv rendering offline state');
    return (
    ...
    if (debug || localDebug) console.log('QRDiv rendering advertising state');
    return (
      <MbfWindow style={enableMobile && mobile ? { flex: '1 1 300px', alignItems: 'center' } : { alignItems: 'center', width: '302px', minWidth: '302px' }}>
        <MbfWindowHeader>
          ...
          <MbfWindowIcons>
            <IconButton aria-label="send advertising" size="small" onClick={handleAdvertiseClick} sx={{ color: 'var(--header-text-color)', margin: '0px', padding: '0px' }}>
              <Tooltip title="Send again the mDNS advertisement" arrow>
                <Icon path={mdiRestart} size="22px" />
              </Tooltip>
            </IconButton>
            <IconButton aria-label="stop pairing" size="small" onClick={handleStopCommissioningClick} sx={{ color: 'var(--header-text-color)', margin: '0px', padding: '0px' }}>
              <Tooltip title="Turn off pairing" arrow>
                <Icon path={mdiShareOffOutline} size="22px" />
              </Tooltip>
            </IconButton>
          </MbfWindowIcons>
        </MbfWindowHeader>
        ...
        <MbfWindowFooter style={{ justifyContent: 'space-between' }}>
          <MbfWindowFooterText style={{ fontWeight: 'normal', color: 'var(--div-text-color)' }}>
    ...
    if (debug || localDebug) console.log('QRDiv rendering commissioned state');
    return (
      <MbfWindow style={enableMobile && mobile ? { flex: '1 1 300px', alignItems: 'center' } : { alignItems: 'center', width: '302px', minWidth: '302px', overflow: 'hidden' }}>
          ...
          <MbfWindowIcons>
            ...
            <IconButton aria-label="start pairing" size="small" onClick={handleStartCommissioningClick} sx={{ color: 'var(--header-text-color)', margin: '0px', padding: '0px' }}>
              <Tooltip title="Turn on pairing" arrow>
                <Icon path={mdiShareOutline} size="22px" />
              </Tooltip>
            </IconButton>
          </MbfWindowIcons>
        </MbfWindowHeader>
        ...
        <MbfWindowContent style={{ flexDirection: 'column', justifyContent: 'flex-start', flex: '1 1 auto', overflow: 'auto', padding: '0px 0px 10px 0px', gap: '0px' }}>
          {matter.fabricInformations.map((fabric, index) => (
            <div
              key={index}
              style={{ margin: '0px', padding: '10px', gap: '0px', color: 'var(--div-text-color)', backgroundColor: 'var(--div-bg-color)', textAlign: 'left', fontSize: '14px' }}
            >
    ...
    if (debug || localDebug) console.log('QRDiv rendering not commissioned and not advertising state');
    return (
      <MbfWindow style={enableMobile && mobile ? { flex: '1 1 300px', alignItems: 'center' } : { alignItems: 'center', width: '302px', minWidth: '302px' }}>
        <MbfWindowHeader>
          <MbfWindowHeaderText>QR pairing code</MbfWindowHeaderText>
        ...
        <MbfWindowText style={{ maxWidth: '280px', fontWeight: 'bold', color: 'var(--secondary-color)' }}>{matter.id}</MbfWindowText>
        <Button
          onClick={handleStartCommissioningClick}
          endIcon={<Icon path={mdiShareOutline} size={1} />}
          style={{ margin: '20px', color: 'var(--main-button-color)', backgroundColor: 'var(--main-button-bg-color)', height: '30px', minWidth: '90px' }}
        >
          Turn on pairing
        </Button>
        <MbfWindowFooter style={{ justifyContent: 'center', height: 'auto', minHeight: '30px' }}>
    ...
    if (debug || localDebug) console.log('QRDiv rendering unknown state');
    return null;
  }
}
