...
describe('Matter clusters revision (guard against upstream changes)', () => {
  ...
  test.each(cases)(`Cluster %s revision has changed`, (_name, entry, expected) => {
    const actual = getClusterRevision(entry);
    if (actual !== expected) {
      failures.push(`Cluster ${_name} revision changed: expected=${String(expected)} actual=${String(actual)}`);
    }
  ...
  afterAll(() => {
    if (failures.length > 0) {
      ...
      console.warn('Cluster revision changes detected:\n' + failures.join('\n'));
    }
  });
});
