...
describe('Matterbridge ' + NAME, () => {
  ...
  test('create a oven device', async () => {
    ...
    expect(device.getAllClusterServerNames()).toEqual(['descriptor', 'matterbridge', 'identify', 'powerSource']);
  ...
  test('device forEachAttribute', async () => {
    ...
    expect(attributes.length).toBe(33); // 33 attributes for the oven device
  });
