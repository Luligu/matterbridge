...
- [CameraAvSettingsUserLevelManagement]: Add the `DigitalPTZ` feature to `MatterbridgeCameraAvSettingsUserLevelManagementServer`: the `DPTZStreams` attribute and the `DPTZSetViewport` and `DPTZRelativeMove` commands, implemented per Matter 1.6.0 § 11.3.7.6 and § 11.3.7.7. `DPTZRelativeMove` clips the resulting viewport to the aspect ratio of the stream, then to `MinViewportResolution`, then to the sensor size, and finally back inside the sensor Cartesian plane, in the order the specification lists. `DPTZStreams` is kept in step with the `CameraAvStreamManagement` cluster of the same endpoint by reacting to its `AllocatedVideoStreams` and `Viewport` attributes, so an entry is seeded from the global `Viewport` when a video stream is allocated (Matter 1.6.0 § 11.3.6.4), dropped when it is deallocated, and re-based when the global `Viewport` changes (Matter 1.6.0 § 11.2.7.25) — including for the video streams that `MatterbridgeWebRtcTransportProviderServer` allocates on its own.
