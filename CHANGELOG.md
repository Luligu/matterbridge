...
- [Oven]/[Refrigerator]: `addCabinet()` accepts new optional `criticalOverTemperatureThreshold`/`criticalUnderTemperatureThreshold` fields on `OvenCabinetOptions`/`RefrigeratorCabinetOptions`; passing either adds a TemperatureAlarm cluster server to the cabinet endpoint (omitting both leaves it absent, matching the cluster's optional status on the TemperatureControlledCabinet device type).
