...
- [plugins]: The plugin frontend can fetch from the platform with a full REST API: /plugins/plugin-name/api/:var. It will call platform.onFetch(method: string, path?: string, query?: Record<string, unknown>, body?: unknown) (for an example check matterbridge-test). (https://github.com/Luligu/matterbridge/issues/561).
