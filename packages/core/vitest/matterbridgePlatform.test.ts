...
describe('Matterbridge platform', () => {
  ...
  const serverLog = new AnsiLogger({ logName: 'BroadcastServer', logTimestampFormat: TimestampFormat.TIME_MILLIS, logLevel: LogLevel.DEBUG });

  // The 'matterbridge' responder updates the shared instance when checkUpdates broadcasts the resolved versions.
  const matterbridgeServer = new BroadcastServer('matterbridge', serverLog);
  ...
  const pluginsServer = new BroadcastServer('plugins', serverLog);
  pluginsServer.on('broadcast_message', (msg: WorkerMessage) => {
    if (msg.type === 'plugins_getschema') {
      pluginsServer?.respond({ type: 'plugins_getschema', src: 'plugins', dst: msg.src, id: msg.id, timestamp: msg.timestamp, result: { schema: {} } });
    }
  });
  ...
  afterAll(async () => {
    // Close the BroadcastServer responders
  ...
  test('checkEndpointNumbers should be empty', async () => {
    ...
    platform.log.logLevel = LogLevel.DEBUG;
  });
  ...
  test('checkEndpointNumbers should not be empty', async () => {
    const testDevice = new MatterbridgeEndpoint(contactSensor, { id: 'test', number: EndpointNumber(100) }, true);
    testDevice.createDefaultBasicInformationClusterServer('test', 'serial01234');
    testDevice.addRequiredClusterServers();
    await platform.registerDevice(testDevice);
  ...
  test('checkEndpointNumbers should check the testDevice with child endpoints', async () => {
    ...
    await platform.registerDevice(testDevice);
    await testDevice.construction.ready;
    expect(testDevice.getChildEndpoints()).toHaveLength(3);
    vi.clearAllMocks();
    expect(await (platform as any).checkEndpointNumbers()).toBe(4);
    expect(loggerLogSpy).toHaveBeenCalledWith(LogLevel.DEBUG, `Setting endpoint number for device ${CYAN}${testDevice.uniqueId}${db} to ${CYAN}${testDevice.maybeNumber}${db}`);
  ...
  test('checkEndpointNumbers should check the testDevice with child endpoints and log change', async () => {
    await platform.unregisterAllDevices();
    const testDevice = new MatterbridgeEndpoint(contactSensor, { id: 'test', number: EndpointNumber(101) }, true);
    testDevice.createDefaultBasicInformationClusterServer('test', 'serial01234');
    testDevice.addRequiredClusterServers();
    ...
    expect(await (platform as any).checkEndpointNumbers()).toBe(4);
    expect(loggerWarnSpy).toHaveBeenCalled();
    expect(loggerDebugSpy).not.toHaveBeenCalledWith(`Setting child endpoint number for device ${CYAN}${testDevice.uniqueId}${db}.${CYAN}child1${db} to ${CYAN}202${db}`);
    expect(loggerDebugSpy).not.toHaveBeenCalledWith(`Setting child endpoint number for device ${CYAN}${testDevice.uniqueId}${db}.${CYAN}child2${db} to ${CYAN}203${db}`);
    await platform.unregisterDevice(testDevice);
    await platform.unregisterAllDevices();
  });
  ...
  test('getSchema', async () => {
    expect(await platform.getSchema()).toEqual({});
  });
  ...
  test('onFetch should log a message and return undefined when not overridden', async () => {
    expect(await platform.onFetch('GET')).toBeUndefined();
    expect(await platform.onFetch('GET', 'myVar')).toBeUndefined();
    expect(await platform.onFetch('POST', 'myVar', { filter: 'a' })).toBeUndefined();
    expect(await platform.onFetch('PUT', 'myVar', { filter: 'a' }, { key: 'value' })).toBeUndefined();
    expect(loggerLogSpy).toHaveBeenCalledWith(LogLevel.DEBUG, expect.stringContaining(`onFetch called:`));
  });
