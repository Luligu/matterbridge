...
const wssSendSnackbarMessageFrontendSpy = vi.spyOn(Frontend.prototype, 'wssSendSnackbarMessage');
const wssSendCloseSnackbarMessageFrontendSpy = vi.spyOn(Frontend.prototype, 'wssSendCloseSnackbarMessage');
const wssSendRefreshRequiredFrontendSpy = vi.spyOn(Frontend.prototype, 'wssSendRefreshRequired');
const wssSendRestartNotRequiredFrontendSpy = vi.spyOn(Frontend.prototype, 'wssSendRestartNotRequired');
const installPluginSpy = vi.spyOn(PluginManager.prototype, 'install');
const uninstallPluginSpy = vi.spyOn(PluginManager.prototype, 'uninstall');

// Spy on Matterbridge methods
const createServerNodeSpy = vi.spyOn(Matterbridge.prototype as any, 'createServerNode');
const startServerNodeSpy = vi.spyOn(Matterbridge.prototype as any, 'startServerNode');
...
describe('Matterbridge frontend', () => {
  ...
  test('Add mock plugin 1', async () => {
    await matterbridge.plugins.add('./packages/core/src/mock/plugin1');
  ...
  test('Add mock plugin 2', async () => {
    await matterbridge.plugins.add('./packages/core/src/mock/plugin2');
    const plugins: Plugin[] = matterbridge.plugins.array();
    expect(plugins).toBeDefined();
  ...
  test('Add mock plugin 3', async () => {
    await matterbridge.plugins.add('./packages/core/src/mock/plugin3');
    const plugins: Plugin[] = matterbridge.plugins.array();
    expect(plugins).toBeDefined();
  ...
  test('Websocket API send /api/restart', async () => {
    const restart = vi.spyOn(matterbridge as any, 'restartProcess').mockImplementationOnce(() => {
  ...
  test('Websocket API send /api/shutdown', async () => {
    const shutdown = vi.spyOn(matterbridge as any, 'shutdownProcess').mockImplementationOnce(() => {
      // Simulate a successful command execution
    });
  ...
  test('Websocket API send /api/create-matterbridge-storage-backup', async () => {
    vi.spyOn(frontend, 'zip').mockImplementationOnce(() => {
  ...
  test('Websocket API send /api/unregister', async () => {
    const unregister = vi.spyOn(matterbridge as any, 'unregisterAndShutdownProcess').mockImplementationOnce(() => {
      // Simulate a successful command execution
    });
  ...
  test('Websocket API send /api/reset', async () => {
    const reset = vi.spyOn(matterbridge as any, 'shutdownProcessAndReset').mockImplementationOnce(() => {
      // Simulate a successful command execution
    });
  ...
  test('Websocket API send /api/factoryreset', async () => {
    const reset = vi.spyOn(matterbridge as any, 'shutdownProcessAndFactoryReset').mockImplementationOnce(() => {
      // Simulate a successful command execution
    });
  ...
  test('Websocket API send /api/devices with params', async () => {
    const msg = await waitMessageId(++WS_ID, '/api/devices', {
      ...
      src: 'Jest test',
      method: '/api/devices',
      params: { pluginName: 'matterbridge-mock1' },
    });
  ...
  test('Websocket API send /api/clusters with wrong params', async () => {
    const msg = await waitMessageId(++WS_ID, '/api/clusters', {
      id: WS_ID,
      dst: 'Matterbridge',
      src: 'Jest test',
      method: '/api/clusters',
      params: { plugin: 'matterbridge-mock1' },
    });
  ...
  test('Websocket API send /api/clusters with wrong endpoint', async () => {
    const msg = await waitMessageId(++WS_ID, '/api/clusters', {
      ...
      src: 'Jest test',
      method: '/api/clusters',
      params: { plugin: 'matterbridge-mock1', endpoint: 15 },
    });
  ...
  test('Websocket API send /api/clusters', async () => {
    const msg = await waitMessageId(++WS_ID, '/api/clusters', {
      ...
      src: 'Jest test',
      method: '/api/clusters',
      params: { plugin: 'matterbridge-mock1', endpoint: 7 },
    });
  ...
  test('Websocket API send /api/select/devices with wrong plugin', async () => {
    const msg = await waitMessageId(++WS_ID, '/api/select/devices', {
      id: WS_ID,
      dst: 'Matterbridge',
      src: 'Jest test',
      method: '/api/select/devices',
      params: { plugin: 'matterbridge_unknown' },
    });
  ...
  test('Websocket API send /api/select/entities with wrong plugin', async () => {
    const msg = await waitMessageId(++WS_ID, '/api/select/entities', {
      id: WS_ID,
      dst: 'Matterbridge',
      src: 'Jest test',
  ...
  test('Websocket API /api/install', async () => {
    ...
    expect((matterbridge as any).plugins.size).toBe(3);
    const msg = await waitMessageId(++WS_ID, '/api/install', {
      id: WS_ID,
      dst: 'Matterbridge',
      src: 'Jest test',
      method: '/api/install',
      params: { packageName: 'matterbridge-test', restart: false },
    });
  ...
  test('Websocket API /api/uninstall with wrong params', async () => {
    const msg = await waitMessageId(++WS_ID, '/api/uninstall', {
      id: WS_ID,
      dst: 'Matterbridge',
      src: 'Jest test',
      method: '/api/uninstall',
      params: { packageName: 'matterbri' },
    });
    expect(msg.response).not.toBeDefined();
  ...
  test('Websocket API /api/uninstall', async () => {
    ...
    const msg = await waitMessageId(++WS_ID, '/api/uninstall', {
      ...
      src: 'Jest test',
      method: '/api/uninstall',
      params: { packageName: 'matterbridge-test' },
    });
    await flushAsync(undefined, undefined, 100);
    expect(msg.success).toBe(true);
    expect(msg.error).not.toBeDefined();
  ...
  test('Websocket API /api/addplugin', async () => {
    const pluginNameOrPath = path.join('.', 'packages', 'core', 'src', 'mock', 'plugin4');
    ...
    await waitMessageId(++WS_ID, '/api/addplugin', { id: WS_ID, dst: 'Matterbridge', src: 'Jest test', method: '/api/addplugin', params: { pluginNameOrPath } });
    await waitMessageId(++WS_ID, '/api/addplugin', {
      id: WS_ID,
      dst: 'Matterbridge',
      src: 'Jest test',
      method: '/api/addplugin',
      params: { pluginNameOrPath: 'matterbridge-mock4' },
    });
  });
  ...
  test('Websocket API /api/savepluginconfig', async () => {
    ...
    const data = await waitMessageId(++WS_ID, '/api/savepluginconfig', {
      id: WS_ID,
      dst: 'Matterbridge',
      src: 'Jest test',
      method: '/api/savepluginconfig',
      params: { pluginName, formData },
    });
    expect(data.error).toBeUndefined();
    expect(data.response).toBeUndefined();
    expect(data.success).toBe(true);
    ...
    // Wrong parameters
    await waitMessageId(++WS_ID, '/api/savepluginconfig', {
      id: WS_ID,
      dst: 'Matterbridge',
      src: 'Jest test',
      method: '/api/savepluginconfig',
    ...
    await waitMessageId(++WS_ID, '/api/savepluginconfig', {
      ...
      src: 'Jest test',
      method: '/api/savepluginconfig',
      params: { pluginName: 'matterbridge-notvalid', formData },
    });
  });
  ...
  test('Websocket API /api/action with wrong plugin', async () => {
    const data = await waitMessageId(++WS_ID, '/api/action', {
      id: WS_ID,
      dst: 'Matterbridge',
      src: 'Jest test',
      method: '/api/action',
      params: { plugin: 'xyz', action: 'test' },
    });
    ...
    // Wrong parameters
    await waitMessageId(++WS_ID, '/api/action', {
      id: WS_ID,
      dst: 'Matterbridge',
      src: 'Jest test',
      method: '/api/action',
      params: { plugin: 'matterbridge-notvalid', action: 'test' },
    });
  });
  ...
  test('Websocket API /api/action with throw', async () => {
    ...
    vi.spyOn(plugin.platform, 'onAction').mockImplementationOnce(async () => {
    ...
    const data = await waitMessageId(++WS_ID, '/api/action', {
      ...
      src: 'Jest test',
      method: '/api/action',
      params: { plugin: 'matterbridge-mock2', action: 'test' },
    });
    expect(data.success).toBeUndefined();
    expect(data.response).toBeUndefined();
    expect(data.error).toBeDefined();
  ...
  test('Websocket API /api/config with invalid values does not change config', async () => {
    const ctx = matterbridge.nodeContext;
    // Cases that take no action and send no response on an invalid value: capture the value before, assert it is unchanged after.
    const noResponseCases: { name: string; value: unknown; key: string }[] = [
      { name: 'setpassword', value: 123, key: 'password' },
      { name: 'setbridgemode', value: 'invalidmode', key: 'bridgeMode' },
      { name: 'setmbloglevel', value: 'ab', key: 'matterbridgeLogLevel' },
      { name: 'setmblogfile', value: 'notbool', key: 'matterbridgeFileLog' },
      { name: 'setmjloglevel', value: 'ab', key: 'matterLogLevel' },
      { name: 'setmjlogfile', value: 'notbool', key: 'matterFileLog' },
      { name: 'setmdnsinterface', value: 123, key: 'mattermdnsinterface' },
      { name: 'setipv4address', value: 123, key: 'matteripv4address' },
      { name: 'setipv6address', value: 123, key: 'matteripv6address' },
    ];
    const before = new Map<string, unknown>();
    for (const c of noResponseCases) before.set(c.key, await ctx?.get(c.key));
    for (const c of noResponseCases) {
      ws.send(JSON.stringify({ id: ++WS_ID, dst: 'Matterbridge', src: 'Jest test', method: '/api/config', params: { name: c.name, value: c.value } }));
    }

    // Numeric (non-string) values take the cond-expr false side (parsed value 0). They always respond (success or reset error).
    // Awaiting these also guarantees the fire-and-forget messages above were processed (in-order delivery).
    for (const name of ['setmatterport', 'setmatterdiscriminator', 'setmatterpasscode']) {
      ws.send(JSON.stringify({ id: ++WS_ID, dst: 'Matterbridge', src: 'Jest test', method: '/api/config', params: { name, value: 5550 } }));
      const data = await waitMessageId(WS_ID, '/api/config');
      expect(data.id).toBe(WS_ID);
    }

    // setvirtualmode always responds, even when the value is invalid (the guard just skips the state change).
    const virtualBefore = await ctx?.get('virtualmode');
    ws.send(JSON.stringify({ id: ++WS_ID, dst: 'Matterbridge', src: 'Jest test', method: '/api/config', params: { name: 'setvirtualmode', value: 123 } }));
    const vData = await waitMessageId(WS_ID);
    expect(vData.success).toBe(true);
    expect(await ctx?.get('virtualmode')).toBe(virtualBefore);

    for (const c of noResponseCases) expect(await ctx?.get(c.key)).toBe(before.get(c.key));
  }, 60000);

  test('Websocket API /api/config with empty mdns/ip values resets to all interfaces/addresses', async () => {
    for (const name of ['setmdnsinterface', 'setipv4address', 'setipv6address']) {
      ws.send(JSON.stringify({ id: ++WS_ID, dst: 'Matterbridge', src: 'Jest test', method: '/api/config', params: { name, value: '' } }));
      const data = await waitMessageId(WS_ID, '/api/config');
      expect(data.success).toBe(true);
    }
  ...
  test('Websocket SendMessage', async () => {
    const spyWssBroadcastMessage = vi.spyOn(Frontend.prototype, 'wssBroadcastMessage');
    ...
    matterbridge.frontend.wssSendLogMessage(
      ...
      '12:45',
      'Frontend',
      `***${CYAN}Test message 01234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789${rs}`,
    );
  ...
  test('Remove mock plugin 1', async () => {
    expect((matterbridge as any).plugins.has('matterbridge-mock1')).toBeTruthy();
    await (matterbridge as any).plugins.remove('./packages/core/src/mock/plugin1');
  ...
  test('Remove mock plugin 2', async () => {
    expect((matterbridge as any).plugins.has('matterbridge-mock2')).toBeTruthy();
    await (matterbridge as any).plugins.remove('./packages/core/src/mock/plugin2');
  ...
  test('Remove mock plugin 3', async () => {
    expect((matterbridge as any).plugins.has('matterbridge-mock3')).toBeTruthy();
    await (matterbridge as any).plugins.remove('./packages/core/src/mock/plugin3');
