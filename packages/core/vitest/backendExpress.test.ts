...
const mockedBackend = {
  ...
  backendWsServer: {
    ...
    wssSendCloseSnackbarMessage: vi.fn(),
  },
  generateDiagnostic: vi.fn(async () => {
...
describe('BackendExpress', () => {
  ...
  test('BroadcastServer handler', async () => {
    const isWorkerRequestSpy = vi.spyOn((backendExpress as any).server, 'isWorkerRequest').mockReturnValue(true);
  ...
  test('validateReq uses req.url when req.originalUrl is undefined', () => {
    ...
    const warnSpy = vi.spyOn((backendExpress as any).log, 'warn');
    const res = {
      status: vi.fn(() => res),
      json: vi.fn(() => res),
    } as any;
