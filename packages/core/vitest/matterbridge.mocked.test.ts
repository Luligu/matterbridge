...
import type { DeviceManager as DeviceManagerType } from '../src/deviceManager.js';
import type { Matterbridge as MatterbridgeType } from '../src/matterbridge.js';
import type { Plugin, PluginManager as PluginManagerType } from '../src/pluginManager.js';
...
vi.mock('@matterbridge/utils/wait', async () => {
  const actual = await vi.importActual<typeof import('@matterbridge/utils/wait')>('@matterbridge/utils/wait');
  return {
    ...actual,
    waiter: vi.fn(
    ...
    wait: vi.fn(async (timeout: number = 1000, name?: string, debug: boolean = false) => {
      return Promise.resolve(); // Mock the wait function to resolve immediately
    }),
    withTimeout: vi.fn(async (promise: Promise<any>, timeoutMillisecs: number = 10000, reThrow: boolean = true) => {
...
const wait = waitModule.wait as MockedFunction<typeof waitModule.wait>;
const waiter = waitModule.waiter as MockedFunction<typeof waitModule.waiter>;
const withTimeout = waitModule.withTimeout as MockedFunction<typeof waitModule.withTimeout>;

vi.mock('@matterbridge/utils/npm-prefix', async () => {
  const actual = await vi.importActual<typeof import('@matterbridge/utils/npm-prefix')>('@matterbridge/utils/npm-prefix');
  return {
    ...actual,
    getGlobalNodeModules: vi.fn(async () => Promise.resolve('usr/local/lib/node_modules')),
  };
});
const npmPrefixModule = await import('@matterbridge/utils/npm-prefix');
const getGlobalNodeModulesMock = npmPrefixModule.getGlobalNodeModules as MockedFunction<typeof npmPrefixModule.getGlobalNodeModules>;

vi.mock('@matterbridge/utils/network', async () => {
  const actual = await vi.importActual<typeof import('@matterbridge/utils/network')>('@matterbridge/utils/network');
  return {
    ...actual,
    logInterfaces: vi.fn(() => {}),
  };
...
const logInterfacesMock = networkModule.logInterfaces as MockedFunction<typeof networkModule.logInterfaces>;

// Mock the createESMWorker from workers module before importing it
vi.mock('@matterbridge/thread', () => ({
  createESMWorker: vi.fn(() => {
    return; // Mock the createESMWorker function to return immediately
  }),
}));

// Mock the exec function from the child_process module so loading does not run real commands.
...
const execSyncMock = childprocessModule.execSync as MockedFunction<typeof childprocessModule.execSync>;
...
describe('Matterbridge mocked', () => {
  ...
  test('Matterbridge.initialize()', async () => {
    const { MatterbridgeEndpoint } = await import('../src/matterbridgeEndpoint.js');
  ...
  test('Matterbridge.initialize() with pairing.json', async () => {
    ...
    vi.spyOn(matterbridge as any, 'parseCommandLine').mockImplementationOnce(async () => {
      return Promise.resolve();
    });
    process.argv = [
      'node',
  ...
  test('Matterbridge.initialize() mdnsinterface', async () => {
    const logNodeSpy = vi.spyOn(matterbridge as any, 'logNodeAndSystemInfo').mockImplementation(async () => {
      return Promise.resolve();
    });
    const parseCommandLineSpy = vi.spyOn(matterbridge as any, 'parseCommandLine').mockImplementation(async () => {
      return Promise.resolve();
    });
  ...
  test('Matterbridge.initialize() plugins', async () => {
    ...
    expect(await (matterbridge.plugins as any).add('./packages/core/src/mock/plugin1')).not.toBeNull();
    expect(await (matterbridge.plugins as any).add('./packages/core/src/mock/plugin2')).not.toBeNull();
    expect(await (matterbridge.plugins as any).add('./packages/core/src/mock/plugin3')).not.toBeNull();
    expect(await (matterbridge.plugins as any).add('./packages/core/src/mock/plugin4')).not.toBeNull();
    expect(await (matterbridge.plugins as any).add('./packages/core/src/mock/plugin5')).not.toBeNull();
    expect(await (matterbridge.plugins as any).add('./packages/core/src/mock/plugin6')).not.toBeNull();
  ...
  test('Matterbridge.initialize() reinstall of plugins', async () => {
    ...
    const parseSpy = vi.spyOn(PluginManager.prototype, 'parse').mockImplementation(async (plugin: Plugin | string) => {
      return null; // Simulate a plugin that does not return a valid instance
    });
    const existSpy = vi.spyOn(fs, 'existsSync').mockImplementation((path: PathLike) => {
    ...
    execSyncMock.mockImplementation((command: string, options?: ExecSyncOptions) => {
      if (command.includes('npm install')) {
        // console.log('Mocked npm install command:', command, options);
        if (command.includes('matterbridge-mock6')) throw new Error('Simulated npm install failure'); // Simulate a not successful npm install for one plugin
        return command; // Simulate a successful npm install command
      }
      if (command.includes('npm link matterbridge')) {
        // console.log('Mocked npm link command:', command, options);
        if (options?.cwd.includes('plugin5')) throw new Error('Simulated npm link failure'); // Simulate a not successful npm link for one plugin
        return command; // Simulate a successful npm link command
      }
    ...
    clearInterval((matterbridge as any).checkUpdateInterval);
    expect(plugins.length).toBe(6);
    expect(existSpy).toHaveBeenCalled(); // Six plugins checked for existence
    expect(parseSpy).toHaveBeenCalled(); // One plugin is skipped due to mocked invalid install
    expect(execSyncMock).toHaveBeenCalled(); // Six plugins with npm install and six plugins with npm link

    expect(loggerInfoSpy).toHaveBeenCalledWith(expect.stringContaining('Matterbridge linked to plugin'));
    expect(loggerErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Error linking matterbridge to plugin'));

    expect(loggerInfoSpy).toHaveBeenCalledWith(expect.stringContaining('Trying to reinstall it from npm...'));
    expect(loggerInfoSpy).toHaveBeenCalledWith(expect.stringContaining('reinstalled'));
    ...
    // Restore the original execSync implementation
  ...
  test('Matterbridge.initialize() startPlugins', async () => {
    // Test startPlugins
    const resolveSpy = vi.spyOn(PluginManager.prototype, 'resolve').mockImplementation(async (pluginPath: string) => {
  ...
  test('Matterbridge.initialize() -service should fails for createStorage', async () => {
    vi.spyOn(NodeStorageManager.prototype, 'createStorage').mockImplementationOnce(() => {
  ...
  test('Matterbridge.initialize() Matter Storage', async () => {
    vi.spyOn(Matterbridge.prototype as any, 'startMatterStorage').mockImplementationOnce(async () => {
  ...
  test('Matterbridge.initialize() update', async () => {
    ...
    vi.useFakeTimers();
    await (matterbridge as any).initialize();
    vi.advanceTimersByTime(12 * 60 * 60 * 1000); // Simulate 12 hours
    vi.useRealTimers();
  ...
  test('Matterbridge.initialize() registerProcessHandlers and matter file logger', async () => {
    ...
    vi.spyOn(matterbridge as any, 'cleanup').mockImplementationOnce(async () => {
      return Promise.resolve();
    });
  ...
  test('Matterbridge.initialize() logNodeAndSystemInfo networkInterfaces', async () => {
    ...
    const networkInterfacesSpy = vi.spyOn(os, 'networkInterfaces').mockReturnValueOnce(ifaces);
  ...
  test('Matterbridge.initialize() parseCommandLine', async () => {
    ...
    // const startMatterStorageSpy = vi.spyOn(matterbridge as any, 'startMatterStorage').mockImplementation(async () => Promise.resolve());
    // const stopMatterStorageSpy = vi.spyOn(matterbridge as any, 'stopMatterStorage').mockImplementation(async () => Promise.resolve());
  ...
  test('Matterbridge.initialize() startBridge and startChildbridge', async () => {
    ...
    clearTimeout((matterbridge as any).checkUpdateTimeout);
    clearInterval((matterbridge as any).checkUpdateInterval);
    const frontendStartSpy = vi.spyOn(matterbridge.frontend, 'start').mockImplementation(async () => Promise.resolve());
    const startBridgeSpy = vi.spyOn(matterbridge as any, 'startBridge').mockImplementation(async () => Promise.resolve());
    const startChildBridgeSpy = vi.spyOn(matterbridge as any, 'startChildbridge').mockImplementation(async () => Promise.resolve());
    expect(matterbridge.nodeContext).toBeDefined();
  ...
  test('Matterbridge.initialize() unregisterAndShutdownProcess', async () => {
    ...
    clearTimeout((matterbridge as any).checkUpdateTimeout);
    clearInterval((matterbridge as any).checkUpdateInterval);
    const cleanupSpy = vi.spyOn(matterbridge as any, 'cleanup');
  ...
  test('Matterbridge.initialize() cleanup()', async () => {
    ...
    const pluginsShutdownSpy = vi.spyOn(matterbridge.plugins as any, 'shutdown').mockImplementation(async () => {
      return Promise.resolve();
    });
  ...
  test('Matterbridge.initialize() startBridge()', async () => {
    ...
    vi.useFakeTimers();
    const createServerNodeSpy = vi.spyOn(matterbridge as any, 'createServerNode').mockImplementation(async () => Promise.resolve({ add: vi.fn() }));
    const createAggregatorNodeSpy = vi.spyOn(matterbridge as any, 'createAggregatorNode').mockImplementation(async () => Promise.resolve({}));
    ...
    vi.advanceTimersByTime(1000); // Simulate 1 second for the interval
    ...
    expect(loggerLogSpy).toHaveBeenCalledWith(
      LogLevel.ERROR,
      expect.stringContaining(`Error waiting for plugin ${plg}${plugin1.name}${er} to load and start. Plugin is in error state.`),
    );
  ...
  test('Matterbridge.initialize() startChildbridge()', async () => {
    ...
    vi.advanceTimersByTime(1000); // Simulate 1 second for the start interval
    ...
    vi.advanceTimersByTime(1000); // Advance 1 second for the start interval
    expect(plugin1.error).toBe(true);
    ...
    expect(loggerLogSpy).toHaveBeenCalledWith(
      LogLevel.ERROR,
      expect.stringContaining(`Error waiting for plugin ${plg}${plugin2.name}${er} to load and start. Plugin is in error state.`),
    );

    vi.advanceTimersByTime(1000); // Simulate 1 second for the start interval. It will clear the interval for error state of the first plugin
    expect((matterbridge as any).startMatterInterval).toBeUndefined();
    expect(loggerLogSpy).toHaveBeenCalledWith(LogLevel.ERROR, expect.stringContaining(`The plugin ${plg}${plugin1.name}${er} is in error state.`));
    ...
    vi.advanceTimersByTime(2000); // Simulate 2 seconds for the interval to execute
    expect((matterbridge as any).startMatterInterval).toBeUndefined();
