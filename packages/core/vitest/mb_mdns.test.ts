...
class MockMdns {
  ...
  readonly logDevices = vi.fn();
  readonly sendQuery = vi.fn();
  readonly sendResponse = vi.fn();
  readonly encodeDnsName = vi.fn((value: string) => Buffer.from(value));
  readonly encodeSrvRdata = vi.fn((priority: number, weight: number, port: number, host: string) => Buffer.from(`${priority}:${weight}:${port}:${host}`));
  readonly encodeTxtRdata = vi.fn((values: string[]) => Buffer.from(values.join(',')));
  readonly encodeA = vi.fn((value: string) => Buffer.from(value));
  readonly encodeAAAA = vi.fn((value: string) => Buffer.from(value));
