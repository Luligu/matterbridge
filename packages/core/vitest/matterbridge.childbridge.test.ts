...
describe('Matterbridge loadInstance() and cleanup() -childbridge mode', () => {
  ...
  test('Matterbridge.loadInstance(true) -childbridge mode', async () => {
    ...
    expect(loggerLogSpy).toHaveBeenCalledWith(
      LogLevel.INFO,
      `The frontend http server is listening on ${UNDERLINE}http://${matterbridge.systemInformation.ipv4Address}:8802${UNDERLINEOFF}${rs}`,
    );
  ...
  test('addBridgedEndpoint twice for AccessoryPlatform', async () => {
    ...
    expect(await plugins.remove('./packages/core/src/mock/plugin4')).not.toBeNull();
  ...
  test('addBridgedEndpoint for AccessoryPlatform with mode = matter', async () => {
    ...
    if (!plugin) return;
    plugin.type = 'AccessoryPlatform';
    plugin.serverNode = { add: vi.fn() } as any;
  ...
  test('addVirtualEndpoint', async () => {
    ...
    expect(await matterbridge.addVirtualEndpoint('matterbridge-mock1', 'Virtual', 'outlet', {} as any)).toBe(true);
