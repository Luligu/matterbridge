...
export function getMatterbridge(): PlatformMatterbridge {
  const matterbridge: PlatformMatterbridge = {
    ...
    matterbridgeVersion: '3.10.11',
    matterbridgeLatestVersion: '3.10.11',
    matterbridgeDevVersion: '3.10.11',
    frontendVersion: '3.6.2',
    bridgeMode: 'none',
    restartMode: 'none',
...
export async function stopServerNode(ticks: number = 1, microTurns: number = 1, pause: number = 10): Promise<void> {
  ...
  // the Vitest process alive indefinitely after all tests finish.
  await server.close();
