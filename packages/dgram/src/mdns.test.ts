...
describe('Mdns', () => {
  ...
  it('should decode PTR, TXT, SRV, A, AAAA, NSEC, and fallback records', () => {
    ...
    const srv = Buffer.concat([
      mdns.encodeDnsName('foo.local'),
      Buffer.from([0, DnsRecordType.SRV, 0, DnsClass.IN, 0, 0, 0, 1, 0, srvTarget.length + 6]),
      Buffer.from([0, 1, 0, 2, 0, 80]),
      srvTarget,
    ]);
    ...
    const nsec = Buffer.concat([
      mdns.encodeDnsName('foo.local'),
      Buffer.from([0, DnsRecordType.NSEC, 0, DnsClass.IN, 0, 0, 0, 1, 0, nsecDomain.length + 2]),
      nsecDomain,
      Buffer.from([0, 0]),
    ]);
  ...
  it('should decode NSEC record with bitmap data', () => {
    ...
    const nsec = Buffer.concat([
      mdns.encodeDnsName('foo.local'),
      Buffer.from([0, DnsRecordType.NSEC, 0, DnsClass.IN, 0, 0, 0, 1, 0, nsecDomain.length + bitmapData.length]),
      nsecDomain,
      bitmapData,
    ]);
