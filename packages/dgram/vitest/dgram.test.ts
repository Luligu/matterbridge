...
describe('Dgram', () => {
  ...
  test('Send a message successfully', async () => {
    ...
    const sendSpy = vi.spyOn(dgram.socket, 'send').mockImplementation((...args: any[]) => {
  ...
  test('Handle error when sending a message', async () => {
    ...
    const errorSpy = vi.spyOn(dgram, 'onError');
    const message = Buffer.from('Hello');
    dgram.send(message, '127.0.0.1', 12345);
  ...
  test('Handle socket close event', async () => {
    dgram = new Dgram('Dgram', 'udp4');
    const closeSpy = vi.spyOn(dgram, 'onClose');
  ...
  test('Handle socket connect event', async () => {
    dgram = new Dgram('Dgram', 'udp4');
    const connectSpy = vi.spyOn(dgram, 'onConnect');
  ...
  test('Handle socket message event', async () => {
    dgram = new Dgram('Dgram', 'udp4');
    const messageSpy = vi.spyOn(dgram, 'onMessage');
    const message = Buffer.from('Hello');
  ...
  test('Handle socket listening event', async () => {
    dgram = new Dgram('Dgram', 'udp4');
    const listeningSpy = vi.spyOn(dgram, 'onListening');
    const address = { address: '127.0.0.1', family: 'IPv4', port: 12345 };

    vi.spyOn(dgram.socket, 'address').mockReturnValue(address);
  ...
  test('Get IPv4 interface address with interface with internal only addresses', async () => {
    dgram = new Dgram('Dgram', 'udp4');
    vi.spyOn(os, 'networkInterfaces').mockReturnValue({
      internalonly: [{ address: '192.168.1.120', family: 'IPv4', internal: true, netmask: '255.255.255.0', mac: '00:00:00:00:00:00', cidr: null }],
    });
  ...
  test('Handle socket binding failure', async () => {
    dgram = new Dgram('Dgram', 'udp4');
    const errorSpy = vi.spyOn(dgram, 'onError');
    const bindSpy = vi.spyOn(dgram.socket, 'bind').mockImplementation(() => {
