/**
 * @file packages/dgram/vitest/multicast.test.ts
...
describe('Multicast', () => {
  ...
  test('Create the multicast with udp6 and let listenig fail', () => {
    ...
    expect(mcast).toBeInstanceOf(Multicast);
    expect(mcast.socketType).toBe('udp6');
    vi.spyOn(Socket.prototype, 'addMembership').mockImplementationOnce((multicastAddress: string, multicastInterface?: string) => {
  ...
  test('Create ipv6 multicast with specific interfaces', async () => {
    ...
    */
    ...
    const socketBindMock = vi.spyOn(mcast.socket as any, 'bind').mockImplementation((...args: any[]) => {
    ...
    const socketAddressMock = vi.spyOn(mcast.socket, 'address').mockReturnValue({
      address: 'fd78:cbf8:4939:746:698e:b44d:64e6:4fb1',
      family: 'IPv6',
      port: COAP_MULTICAST_PORT,
    });
    const socketSetBroadcastMock = vi.spyOn(mcast.socket as any, 'setBroadcast').mockImplementation(() => {});
    const socketSetTTLMock = vi.spyOn(mcast.socket as any, 'setTTL').mockImplementation(() => {});
    const socketSetMulticastTTLMock = vi.spyOn(mcast.socket as any, 'setMulticastTTL').mockImplementation(() => {});
    const socketSetMulticastLoopbackMock = vi.spyOn(mcast.socket as any, 'setMulticastLoopback').mockImplementation(() => {});
    const socketAddMembershipMock = vi.spyOn(mcast.socket as any, 'addMembership').mockImplementation(() => {});
    const socketSetMulticastInterfaceMock = vi.spyOn(mcast.socket as any, 'setMulticastInterface').mockImplementation(() => {});
    const socketDropMembershipMock = vi.spyOn(mcast.socket as any, 'dropMembership').mockImplementation(() => {});
  ...
  test('onListening skips undefined interface entries', () => {
    mcast = new Multicast('Multicast', COAP_MULTICAST_IPV6_ADDRESS, COAP_MULTICAST_PORT, 'udp6', true, undefined, '::');
    vi.spyOn(os, 'networkInterfaces').mockReturnValue({ empty: undefined });
    const addMembershipSpy = vi.spyOn(mcast.socket as any, 'addMembership').mockImplementation(() => {});
    const setBroadcastSpy = vi.spyOn(mcast.socket as any, 'setBroadcast').mockImplementation(() => {});
    const setTTLSpy = vi.spyOn(mcast.socket as any, 'setTTL').mockImplementation(() => {});
    const setMulticastTTLSpy = vi.spyOn(mcast.socket as any, 'setMulticastTTL').mockImplementation(() => {});
    const setMulticastLoopbackSpy = vi.spyOn(mcast.socket as any, 'setMulticastLoopback').mockImplementation(() => {});
  ...
  test('onListening joins multicast using the first IPv6 address', () => {
    ...
    vi.spyOn(mcast.socket as any, 'setMulticastInterface').mockImplementation(() => {});
  ...
  test.each([
    ...
    mcast = new Multicast('Multicast', COAP_MULTICAST_IPV6_ADDRESS, COAP_MULTICAST_PORT, 'udp6', true, undefined, '::');
    vi.spyOn(os, 'networkInterfaces').mockReturnValue({ eth0: interfaces as any });
    const addMembershipSpy = vi.spyOn(mcast.socket as any, 'addMembership').mockImplementation(() => {});
    vi.spyOn(mcast.socket as any, 'setBroadcast').mockImplementation(() => {});
    vi.spyOn(mcast.socket as any, 'setTTL').mockImplementation(() => {});
    vi.spyOn(mcast.socket as any, 'setMulticastTTL').mockImplementation(() => {});
