...
describe('Inspector', () => {
  ...
  test('start: warns when session already active', async () => {
    ...
    const warnSpy = vi.spyOn(inspector['log'], 'warn');
    inspector.session = {};
  ...
  test('takeHeapSnapshot: warns without active session', async () => {
    const { Inspector } = await import('../src/inspector.js');
    const inspector = new Inspector('Inspector', true, false);
    const warnSpy = vi.spyOn((inspector as any)['log'], 'warn');
  ...
  test('interval: runs and logs, then skips while in progress', async () => {
    ...
    vi.doMock('node:inspector', () => ({ default: {}, Session: FakeSession }));
    vi.doMock('node:fs', () => ({ mkdirSync: () => {} }));
    vi.doMock('../src/commandLine.js', () => ({ getIntParameter: () => 30000 }));

    vi.useFakeTimers();
    const { Inspector } = await import('../src/inspector.js');
    const inspector = new Inspector('Inspector', true, false) as any;
    const debugSpy = vi.spyOn(inspector['log'], 'debug');
    // Make snapshot call a fast no-op
    vi.spyOn(inspector, 'takeHeapSnapshot').mockResolvedValue(undefined as any);
    await inspector.start();
  ...
  test('heap snapshot: write backpressure branch executes', async () => {
    ...
    vi.doMock('../src/commandLine.js', () => ({ getIntParameter: () => {} }));
    const { Inspector } = await import('../src/inspector.js');
    const inspector = new Inspector('Inspector', true, false);
    await inspector.start();
    await inspector.takeHeapSnapshot();
  ...
  test('heap snapshot: stream.end throws and is caught', async () => {
    ...
    const { Inspector } = await import('../src/inspector.js');
    const inspector = new Inspector('Inspector', true, false) as any;
    const errorSpy = vi.spyOn(inspector['log'], 'error');
    await inspector.start();
    await inspector.takeHeapSnapshot();
  ...
  test('interval: logs error when scheduled snapshot throws', async () => {
    ...
    const inspector = new Inspector('Inspector', true, false) as any;
    const errorSpy = vi.spyOn(inspector['log'], 'error');
    vi.spyOn(inspector, 'takeHeapSnapshot').mockRejectedValue(new Error('scheduled-error'));
    await inspector.start();
  ...
  test('start: no interval when value below 30000', async () => {
    ...
    vi.doMock('../src/commandLine.js', () => ({ getIntParameter: () => 1000 }));
    const { Inspector } = await import('../src/inspector.js');
    const inspector = new Inspector('Inspector', true, false);
    await inspector.start();
    await inspector.stop();
  ...
  test('takeHeapSnapshot: logs error when snapshot post returns error', async () => {
    ...
    vi.doMock('node:inspector', () => ({ default: {}, Session: ErrSession }));
    vi.doMock('node:fs', () => ({
      mkdirSync: (_p: string) => {},
  ...
  test('stop: logs error when stopSampling fails', async () => {
    ...
    vi.doMock('node:inspector', () => ({ default: {}, Session: StopErrSession }));
    vi.doMock('node:fs', () => ({
      mkdirSync: (_p: string) => {},
      createWriteStream: () => ({ once: () => {}, write: () => true, end: (cb?: () => void) => cb?.() }),
      writeFileSync: () => {},
    ...
    const inspector = new Inspector('Inspector', true, false);
    await inspector.start();
    const errorSpy = vi.spyOn((inspector as any)['log'], 'error');
    await inspector.stop();
