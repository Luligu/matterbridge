...
import { loggerDebugSpy, loggerErrorSpy, originalProcessArgv, setupTest } from '../../../buntest/bunSetupTest.js';
import { BroadcastServer } from '../src/broadcastServer.js';
import { flushAsync } from '../vitest/flushAsync.js';
...
describe('BroadcastServer', () => {
  ...
  test('should not post message when closed', async () => {
    ...
    await expectRejectsWith(server.fetch({ type: 'jest_simple', src: 'frontend', dst: 'manager' }), /Broadcast channel is closed/);
    loggerErrorSpy.mockClear();
  ...
  test('broadcast: should broadcast a valid simple request message', () => {
    const postMessageSpy = spyOn((server as any).broadcastChannel, 'postMessage');
    server.broadcast({ type: 'jest_simple', src: 'frontend', dst: 'manager' });
  ...
  test('request: should not broadcast an invalid request message', () => {
    const postMessageSpy = spyOn((server as any).broadcastChannel, 'postMessage').mockImplementation(() => {});
  ...
  test('broadcastMessageHandler: should receive a broadcast message', async () => {
    const eventHandler = mock<(msg: WorkerMessage) => void>(() => {});
  ...
  test('fetch: should resolve with correct response without id', async () => {
    ...
    const getUniqueIdSpy = spyOn(server, 'getUniqueId').mockReturnValue(generatedId);
    const requestMsg = { type: 'jest', src: 'frontend', dst: 'manager', params: { userId: 1 } } as const;
  ...
  test('fetch: should reject on error response', async () => {
    ...
    await expectRejectsWith(server.fetch(requestMsg), /Fetch received error response Nope/);
  });
  ...
  test('fetch: should reject malformed response', async () => {
    ...
    const guardSpy = spyOn(server as any, 'isWorkerResponseOfType').mockImplementation((value: any, type: any) => {
      if (value.id === 777777) {
        return true;
      }
    ...
    try {
      await expectRejectsWith(server.fetch(requestMsg), /Fetch received malformed response/);
    } finally {
  ...
  test('fetch: should reject on timeout', async () => {
    ...
    await expectRejectsWith(server.fetch(requestMsg), /Fetch timeout/);
  });
  ...
  test('fetch: should reject on timeout from another thread', async () => {
    ...
    const { BroadcastServer } = await import('../src/broadcastServer.js');
    const testServer = new BroadcastServer('manager', log, NAME);
    await expectRejectsWith(testServer.fetch({ id: 123456, type: 'jest', src: 'frontend', dst: 'manager', params: { userId: 1 } }, 10), /Fetch timeout/);
    testServer.close();
  });
  ...
  test('fetch: should ignore wrong response type', async () => {
    ...
    await expectRejectsWith(server.fetch(requestMsg, 20), /Fetch timeout/);
  });
  ...
  test('broadcast: should log error if the port is closed', async () => {
    const broadcastServerBroadcastSpy = spyOn(BroadcastServer.prototype, 'broadcast');
  ...
  test('request: should log error if the port is closed', async () => {
    const broadcastServerRequestSpy = spyOn(BroadcastServer.prototype, 'request');
    const requestMsg = { id: 654321, timestamp: Date.now(), type: 'jest', src: 'manager', dst: 'manager', params: { userId: 1 } } as const;
    server.request(requestMsg);
    await flushAsync(undefined, undefined, 50);
  ...
  test('respond: should log error if the port is closed', async () => {
    const broadcastServerRespondSpy = spyOn(BroadcastServer.prototype, 'respond');
  ...
  test('broadcast/request/respond: should catch postMessage errors (no throw)', () => {
    ...
    const postMessageSpy = spyOn((isolatedServer as any).broadcastChannel, 'postMessage').mockImplementation(() => {
